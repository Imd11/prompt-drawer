...
APP_PATH="$ROOT_DIR/src-tauri/target/release/bundle/macos/Piqory.app"
DMG_PATH="$ROOT_DIR/src-tauri/target/release/bundle/dmg/Piqory_${APP_VERSION}_aarch64.dmg"
...
if command -v hdiutil >/dev/null 2>&1; then
  ...
  ditto "$APP_PATH" "$tmp_dir/Piqory.app"
  mkdir -p "$(dirname "$DMG_PATH")"
  hdiutil create -volname "Piqory" -srcfolder "$tmp_dir" -ov -format UDZO "$DMG_PATH"
fi
